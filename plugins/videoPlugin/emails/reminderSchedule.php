...
while($row_schedules = $sel_schedules->fetch()){
	...
	if(!empty($schedule_time)){
		...
		if(new DateTime() >= $order_call_time){

		  $db->update("order_schedules",["email"=>1],["id"=>$id]);
		  ...
		  $mail = new PHPMailer;
			...
			$mail->isHTML(true);
	    $mail->Subject = "$site_name: Order Video Session Time Has Been Arrived.";
			$mail->Body = <<<EOT
			<html>
			<head>
			<style>
			...
			<hr>
			<p class='lead'> Order Video Session Time Has Been Arrived. Now You Can Video Call To Buyer Of This Order <a href='$site_url/order_details?order_id=$order_id'>#$order_number</a> </p>
			</center>
			</div>
			</div>
			</body>
	    ...
	    $mail->Subject = "$site_name: Order Video Session Time Has Been Arrived.";
			...
			<hr>
			<p class='lead'> Order Video Session Time Has Been Arrived. Now You And Seller Of This Order <a href='$site_url/order_details?order_id=$order_id'>#$order_number</a> Can Video Call Each Other. </p>
			</center>
			</div>
			</div>
			</body>
