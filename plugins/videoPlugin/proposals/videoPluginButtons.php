<?php if(!isset($_SESSION['seller_user_name'])){ ?>
  ...
  <a href="#" data-toggle="modal" data-target="#login-modal" class="btn btn-order">
    <strong>Order Now (<?= $s_currency; ?><span class="total-proposal_price"><?= $proposal_price; ?></span>)</strong>
  </a>
<?php }else{ ?>
  ...
  <?php }else{ ?>
    ...
    <button type="submit" name="add_order" value="1" class="btn btn-order">
      <strong>Order Now (<?= $s_currency; ?><span class="<?= $priceClass; ?>"><?= $proposal_price; ?></span>)</strong>
    </button>
  <?php } ?>
